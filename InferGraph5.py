...
def run_experiment(model, train_loader, val_loader, test_loader, x):
    ...
    fit_losss=[]
    if model.mode==1:
        fit_losss = model.fit(train_loader, optimizer, crit_mse, crit_c, 400)
            ...
            for th1 in th1_list:  
                for th2 in th2_list:
                    ...
                    for i in range(RUN_TIMES):
                        ...
                        model = GNN5(feature_dim=x.shape[1],out_dim=7, hidden_dim=hidden_dim, create_graph=True,
                    ...
                    print(f'completed {RUN_TIMES},train_size={train_size}, mode={mode}, hs={hidden_dim}, drop={drop}, th1={th1}, th2={th2}, num_centroids={model.num_centroids}, train acc: {np.mean(train_accs):.4f},{np.std(train_accs):.4f}, test acc: {np.mean(test_accs):.4f}, {np.std(test_accs):.4f}, p_norm: {np.mean(p_norms):.4f}, {np.std(p_norms):.4f}')
                    # # # save into csv file
